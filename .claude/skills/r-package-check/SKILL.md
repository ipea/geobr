...
version: 1.0.0
argument-hint: "[--no-check to skip R CMD check; defaults to r-package/]"
...
**Input:** `$ARGUMENTS` — optional flags. The package root is always `r-package/` in this repo.
...
git -C "r-package" status --short man/ NAMESPACE   # any diff = generated docs were stale
```
...
```bash
Rscript -e 'devtools::check("r-package", args = "--as-cran")'
# or: R CMD build r-package && R CMD check --as-cran r-package_*.tar.gz
```
...
```bash
Rscript -e 'covr::package_coverage("r-package")'
```
...
"Review the package source at r-package/"
```
...
Save a report to `quality_reports/audits/r_package_check.md` and present a verdict:

```markdown
## Release Gate — geobr (R) [version]
...
[ ] Test suite green with network available (see the proxy note in MEMORY.md)
[ ] Release tag consistent across its live copies (see data-release-conventions.md §2)
[ ] Python parity checked (/parity-check) or the gap recorded in NEWS.md
```
