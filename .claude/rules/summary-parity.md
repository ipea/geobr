...
- `r-package/NEWS.md` / `python-package/CHANGELOG.md` opening paragraph of a version entry
- `README.md` tagline and section ledes (root, `r-package/`, `python-package/`)
...
- `/deep-audit` — the same inconsistency surviving rounds N and N+2 is escalated to the user rather than patched a third time.
- `/r-package-check` — a CRAN NOTE justified in `cran-comments.md` across two consecutive releases without ever being resolved is surfaced prominently ("NOTE explained twice, never fixed") rather than left to hide behind its recorded justification.

In each case, two strikes means the *artifact is the wrong shape*, not that the wording needs one more touch.

## Cross-references

- `CLAUDE.md` — the surface inventory and the data-architecture summary are both drift-prone paragraphs; re-verify them against disk when adding or removing a skill, rule, or agent.
- [`../skills/commit/SKILL.md`](../skills/commit/SKILL.md) — apply this rule to a commit message summary before pushing.
- [`cross-language-parity.md`](cross-language-parity.md) — the same two-strikes logic applies to a parity gap deferred twice.
